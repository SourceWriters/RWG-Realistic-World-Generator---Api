...
public interface IRegenerationHelper {
     ...
     * 
     *                    The status of this method will contain two different
     *                    status objects. The first one is a normal Status object
     *                    which keeps track of the chunk progress while the other
     *                    object is just a reference which passes the values of the
     *                    current chunk through, basically the second one is the
     *                    same as the one of {@code regenerate(Chunk)} while the
     *                    other one is to keep track how many chunks are done and if
     ...
     */
    @NonNull
    default Chunk[] getChunksAndMerge(@NonNull final Location first, @NonNull final Location second, @NonNull final Chunk... array) {
        final Chunk[] current = getChunks(first, second);
        final Chunk[] output = new Chunk[current.length + array.length];
